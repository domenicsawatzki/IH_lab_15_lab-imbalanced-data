...
def to_snake_case_camel_case(input_string):
    snake_case_string = re.sub('([a-z0-9])([A-Z])', r'\1_\2', input_string).lower()
    return snake_case_string

def transform_words_to_snake_case_camel_case(words):
    snake_case_words = [to_snake_case_camel_case(word) for word in words]
...
def min_max_scaler(df, MinMaxtransformer): 
    try:
        # Check if 'data' is a pandas DataFrame
        if not isinstance(df, pd.DataFrame):
            raise ValueError("Input df must be a pandas DataFrame.")
